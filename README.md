...
2. **种植计划与作物推荐**：认养后制定种植计划，按季节推荐适宜作物，生成预期收获时间线（蔬菜 45 天/水果 90 天/香草 35 天）。**一块地同时只允许存在一条未完成计划（计划/播种/生长/采收）：重复提交直接 409 拒绝且保留原计划，完成并重新认养后才能再建；并发提交由地块行锁 + 数据库部分唯一索引双重保证只有一条成功、不留残记录；制定计划窗口中被占用的地块不可选并显示占用状态。**
...
| 方法 | 路径 | 说明 | 鉴权 |
| --- | --- | --- | --- |
| GET | `/plots` | 地块列表（`?status=`，含 `active_plan` 未完成计划占用信息） | 公开 |
| GET | `/plots/:id` | 地块详情（含 `active_plan` 占用信息） | 公开 |
| POST | `/plots` | 创建地块 | 管理员 |
...
| POST | `/planting-plans` | 创建计划（季节校验 + 收获时间线；地块已有未完成计划返回 409 / code=2010） | 登录 |
